function changewords() {
    ...
    achange("ks ", "x ");
    achange("se ", "ze ");
    achange("nigga", "nigzor");
    achange("nigger", "nigzor");
    achange("negro", "nigzor");
    cchange("ah ", "er ");
    cchange("yeer", "yeah");
    achange("ing ", "in   [^]    ");
    ...
    if (transtype == 2) {
        ...
        textin = textin.replace(/(?:are|is|am) good (me|you|him|her|them|y'all|my|your|his|her|their|our)/g, "defeat $1");
    }
}
...
function changeletters() {
    ...
    if (transtype == 1) return; // don't overdue the lame- > l33t
    ...
    // AI:
    if (transtype == 2) {
        textin = textin.replace(/(?!e)@(\B)/g, "at$1"); //@ at the end of words
        cchange("@", "a");
        textin = textin.replace(/iz(?!e)(\w)/g, "r$1"); // |2
        textin = textin.replace(/iy/g, "ly");
        textin = textin.replace(/(\s|.|,|\?|!)uk/g, "$1" + "lik"); // |_||<3 will mean 'like' instead of 'uke'
        cchange("i-i", "h");
        textin = textin.replace(/i(a|\@)te/g, "late"); // |2
        cchange("eei", "eel");
        cchange("iee", "lee");
        cchange("eio", "elo");
        cchange("wlli", "will");
        ...
        cchange("skllis", "skills"); //had to hardcode this one, bc i can't find any other way.
        cchange("u>", "lp");
...
var transtype = 1; // 1 = toLeet, 2 = toLame
var textin;
...
function achange(t1, t2) {
    if (transtype == 1 && Math.random() <= 0.8) change(t1, t2);
...
const Leet = {
    command(ctx, mode) {
        const text = ctx.message.text.replace(/^(\/|!)(д|d) (1337|leet|lame|l4m3)/ig, "").trim();
        const isLeetMode = !mode || mode == "1337";

        let translate = null;
        if (isLeetMode) translate = this.toLeet(text);
        else translate = this.toLame(text);

        return ctx.reply(
            `[${isLeetMode ? "Leet" : "Lame"}]: ${translate}`
        );
    },
